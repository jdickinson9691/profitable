...
**Covers:** `galaxy.md`, `planet.md`, `mining.md`. Fully as-built, zero blockers. **Schema, Simulation Core, and Parity Validation are done** (agents 38-40); Infrastructure Adapters confirmed not needed; Presentation and Phase Integration remain, and need the Unity Editor (not just `dotnet build`/`test`).
...
- [ ] **Presentation.** Rewrite the Unity equivalents of `MapScene`/`GatherScene`'s galaxy-facing parts (galaxy overview, planet detail, gather action) as real Unity scenes — full rewrite per the GDD's own table, not a mechanical port. Requires the Unity Editor (confirmed installed, 6000.5.6f1) — Editor dependency begins here, same as Migration Phase 1's own Agent 35.
