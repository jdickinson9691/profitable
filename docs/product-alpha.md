...
- [ ] Package/build for Windows and macOS — Windows done: `npm run electron:build:win` produces a working NSIS installer (`release/Profitable Setup 0.1.0.exe`) and unpacked `release/win-unpacked/Profitable.exe`, confirmed to boot and render correctly; unsigned, which is expected (no code-signing cert configured, matching the plan's "internal playtest, not public release" scope). Left unchecked since macOS is still unverified — this machine can't build/run a `.app`.
