...
| Core loop resource | Daily turn allocation | Real-time/timestamps; retuned Fuel carries the "spend it wisely" tension instead | **Resolved — deliberate non-adoption, Fuel retuned as the substitute, design-locked, not yet built** |
| Trading counterparty | Permanent NPC ports | Player listings + `sellToMarket()`/`sellToGlobalMarket()` | **Resolved and built** |
| Planet production | Colonist-driven | Colonist-gated (design-locked, not yet built) | **Gap — fixed at design level, not yet built** |
...
| Planet ownership | Citadels, 6 levels | Citadels, 3 levels (design-locked, not yet built) | **Gap — fixed at design level, deliberately reshaped, not yet built** |
...
3. ~~**Is planet ownership (colonists, citadels, player-created planets) worth pursuing as a real design pass?**~~ **Fully resolved.** Colonists and Citadels got that pass (`planet-ownership.md`) and are design-locked, not yet built. Player-created planets (Genesis Torpedoes) were given their own standalone look and explicitly declined — architectural cost (breaks `galaxy.md`'s fixed-seed invariant) outweighs the payoff (thin without corp/PvP territory stakes, and the Citadel system already covers the underlying "build/expand" itch). Not revisited unless Multiplayer eventually adds real territory contention.
