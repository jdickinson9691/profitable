...
**Retroactive correction (2026-08-03):** `resolveHazard()`'s failure-cost-band lookup (`HAZARD_FAILURE_COST_CURVE.find((entry) => pointsBelow >= entry.minPointsBelow && (entry.maxPointsBelow === null || pointsBelow <= entry.maxPointsBelow))`) had the same class of gap as `getTierColor()`/`getPenaltyMultiplier()` (`profitable-mvp-gdd.md`'s own two correction notes), on `HAZARD_FAILURE_COST_CURVE`'s band boundaries (just above 0, 10, 20, 30, and 40 points below the hazard pass threshold). `pointsBelow = HAZARD_PASS_THRESHOLD - effectiveRoll` is only guaranteed an integer when `HAZARD_SHIP_TIER_MODIFIER`'s roll bonus for the ship's tier is itself a whole number — every configured tier's bonus is currently whole (0/5/10/15/20/25/30), so this never fired in real play, but the constant is typed `number`, not an integer, so nothing in the type system prevented a future tuning pass from reintroducing exactly the failure shape the other two boundary bugs already had. Live in shipped code since this feature's own original implementation (predates this correction by several weeks); never caught by `resolveEncounters.test.ts`'s own "failure cost curve matches the escalating shape" test, which only ever exercises Grey tier (`rollBonus = 0`), keeping `pointsBelow` an integer in every case that test covers.

Found not by a crash in play, but during a **Unity migration parity audit** (2026-08-03) that specifically re-checked every ported tier/breakpoint/threshold comparison across Migration Phase 2 for this exact risk class, per `profitable-unity-migration-gdd.md` §6's "no agent re-derives already-fixed logic from scratch... any comparison against a table/breakpoint/threshold must be checked for the same fractional-gap risk, even if the original TypeScript version never exhibited the bug" rule — the C# port (`unity/ProfitableCore/Simulation/ResolveEncounters.cs`) was found to have faithfully carried the same latent gap over from this TypeScript source, which is what surfaced it here rather than in the Unity code itself. Confirmed via the debug/tuning panel's own registry (`debugTuningRegistry.ts`) that the shipped alpha build's UI cannot currently trigger this: the "Hazard ship tier bonus" row is step-button-only with `step: 1`, so it can only ever produce whole-number values through real player/tester interaction — this was a real, live landmine in the source, but not one reachable through the shipped UI as configured today.

Fixed with the same directional choice as `getTierColor()`/`getPenaltyMultiplier()` (round a gap value down into the less-severe band), with one difference from `getPenaltyMultiplier()`'s own fix: there is no zero-width "no violation" band to protect here, since `resolveHazard()`'s own `passed` check already guarantees `pointsBelow` is strictly positive whenever this lookup runs — so the first band's own floor is "greater than 0" rather than a declared integer minimum, closing the below-band-0 gap the same way every other adjacent pair closes its gap from the high side. Fixed in lockstep in both `src/ships/resolveEncounters.ts` and its C# port. Regression coverage: `tests/ships/resolveEncounters.test.ts`'s new fractional-gap test (5 cases, one per gap, using a temporarily-overridden non-Grey tier bonus) and the matching case-for-case test in `ShipsSimulationTests.cs`. No Travel Encounters milestone claim is retracted — the feature worked correctly for every input actually tested and for every input reachable through the shipped debug panel — but, as with both prior boundary corrections, the verification blind spot (a test suite that never exercised a non-Grey tier's contribution to this specific lookup) is documented rather than silently absorbed.
