...
1. ~~Seasons and emergencies were never implemented — only baseline drift exists as a live map-data layer (bug against Agents 1/11/12/15).~~ **Fixed** (post-milestone bug-fix pass): `src/trading/season.ts`/`emergency.ts` — pure functions of `(planetId, now[, categories])`, zero persisted state (so map GDD §2.2's "always live" property holds trivially), wired into `TradeMapScene.renderPlanet()`'s `classify()` call via a live `effectivePrice` multiplier. Live-verified: two independently-seasoned planets showed correct, distinct cheap/premium classifications, including one case where an active emergency's larger premium correctly overrode its own planet's season-driven discount on the same category. See `src/trading/README.md`'s and `src/presentation/README.md`'s "Galactic Map bug fix" notes for the full writeup.
2. ~~Discovery is never extended by travel — only the 2 bootstrap-hardcoded planets are ever reachable (bug against Agents 22/24).~~ **Fixed** (post-milestone bug-fix pass): `galaxyState.ts` gained a persisted `discoveredPlanetIds` side-table plus `markPlanetDiscovered()`/`getDiscoveredPlanets()`; `TradeMapScene.onResolveArrival()` now calls `markPlanetDiscovered()` on every successful arrival. Live-verified to survive a real page reload. See `src/presentation/README.md`'s "Galactic Map bug fix" note for the full writeup.
3. ~~`TradeMapScene` and the shared nav bar both overflow their 800×500 canvas at the current, real scale, with no scrolling mechanism (bug against Agents 13/18/22).~~ **Fixed** (post-milestone bug-fix pass): `scenes/nav.ts` now wraps to a second row instead of running off the canvas edge; `TradeMapScene.ts`'s content now renders inside a mask-clipped, mouse-wheel-scrollable `Container`, with interactive buttons' own `input.enabled` toggled based on whether they're actually inside the visible viewport (a `GeometryMask` clips rendering only, not input hit-testing, so this was a necessary completion, not optional polish). Live-verified: nav wraps correctly, scrolling reveals previously-unreachable content. See `src/presentation/README.md`'s "Galactic Map bug fix" note for the full writeup.

All three items originally carried forward by this milestone are now fixed — this milestone's job was to verify and report, which is now complete, and the follow-on bug-fix work is also done.
