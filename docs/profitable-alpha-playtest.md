...
3. Click **Debug**, then click **`[ Re-seed playtest save ]`** (top of the panel, under "Playtest setup:"). This is the one action that gets you playtest-ready — it seeds, in one click:
   ...
   - **Inventory**: 40x Igneous Ore + 20x Autunite Crystal (enough raw material for up to 20 Radiant Alloy Bar refines — A1 asks for 5-10 at each of two refiner tiers), plus 3 pre-rolled Radiant Alloy Bar batches at different durability bands (comfortably above threshold, 12 points below, 38 points below) and 6x Hydrogen Gas — all set up so A2's threshold-penalty comparison is runnable immediately with no gather step.
   - **6 additional discovered planets** beyond your starting planet and its one auto-discovered neighbor (8 total). One of them (see A3 below) is deliberately a same-resource-type match at a contrasting tier to your starting neighbor, so A3 is actually runnable — see that scenario's note below for why this needed a specific pick, not just "any nearby planet." Another (see B4 below) is deliberately the single farthest planet from your start in this galaxy, measured, not assumed.
...
This scenario needs two *discovered* planets of the **same planet type** at contrasting tiers, sharing at least one producible resource — your starting planet and its auto-discovered neighbor don't actually satisfy this (they're different planet types with zero resource overlap, an error in an earlier version of this doc's seed comments, corrected — see `devGalaxySeed.ts`'s header comment for the full audit trail). The playtest seed now specifically discovers a real pair that works: a **Gold-tier Gas Giant** and a **Grey-tier Gas Giant**, which share **Hydrogen Gas** and **Helium-3**.
1. Go to **TradeMap**, scroll the discovered-planets list to find the two Gas Giant entries (one Gold tier, one Grey tier).
2. Initiate a voyage to whichever one isn't your current location (Travel section → **`> Initiate Voyage`** next to its name). Wait for it to arrive (or see Step "Time-dependent scenarios" below if you don't want to wait in real time), then **`> Resolve Arrival`**.
3. Go to **Gather** and click **`> Gather Hydrogen Gas`** (or Helium-3) several times; note the quality rolls.
4. Travel to the other Gas Giant (same steps) and gather the same resource there.
5. Compare the two planets' output quality per the plan doc's A3 question.

### A4 — Specialty planet payoff

**Screens: TradeMap → Gather.**
1. Your starting planet's auto-discovered neighbor is a Gold-tier Gas Giant with a real specialty resource (fusion-gas-mix). Travel there (Initiate Voyage → Resolve Arrival, same as A3).
...
A note on the numbers before you start: the plan doc's own "~24-28h" example for a long trip was illustrative, not a guarantee for every generated galaxy — measured directly against this specific playtest seed and starting position, the farthest of all 50 planets from your start tops out around **~14.7h** at Grey tier (this galaxy's starting planet isn't centered in the position range, so its own reachable spread is smaller than the galaxy's absolute maximum). That farthest planet is seeded as one of your discovered planets specifically so this scenario is still runnable with a real, meaningfully-long number — just don't expect the doc's original 24-28h figure to show up on screen.

1. In the Travel section, compare the listed `<h>h` travel time next to your nearest discovered neighbor (should read ~1-4h at Grey tier) against your farthest discovered planet (should read ~13-15h at Grey tier).
2. Initiate the short hop first, resolve it, gather the immediate "was that quick?" impression, then do the long trip and gather the "did that feel like a commitment?" impression. Use the Grey-tier seeded ship for both, so speed tier isn't a confound (that's B5, next). If ~14-15h doesn't feel like enough of a "real commitment" relative to the plan doc's original 24-28h expectation, that itself is worth recording in the Feedback Capture Template — it may mean `DISTANCE_TO_TRAVEL_HOURS_PER_UNIT` or the galaxy's position range needs revisiting, not just this doc's wording.
