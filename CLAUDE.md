...
**Also active now, in parallel: Unity Migration Phase 1.** Scope-locked and ready to build per `docs/profitable-unity-migration-gdd.md` — see Section 2 above for how this relates to Alpha (parallel tracks, neither blocks the other). Agents 31-36 (renumbered from the GDD's original 30-35 draft — Agent 30 was already claimed by `agent-30-alpha-content-confirmation.md`) port the MVP core loop (`rollQuality`, `getTierColor`, `refine`, `craft`, `loadContent`) to C# with numeric-parity validation against the existing TypeScript test suite, then build minimal Unity presentation for gather/refine/craft. Agent 31's contract (`docs/agents/agent-31-unity-data-schema.md`) is written; Agents 32-36's are not yet.
