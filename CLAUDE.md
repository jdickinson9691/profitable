...
**Also active now, in parallel: Unity Migration Phase 1.** Scope-locked and ready to build per `docs/profitable-unity-migration-gdd.md` — see Section 2 above for how this relates to Alpha (parallel tracks, neither blocks the other). Agents 30-35 port the MVP core loop (`rollQuality`, `getTierColor`, `refine`, `craft`, `loadContent`) to C# with numeric-parity validation against the existing TypeScript test suite, then build minimal Unity presentation for gather/refine/craft. Individual agent contracts (`docs/agents/agent-30-*.md` through `agent-35-*.md`) are not written yet.
...
- `docs/profitable-unity-migration-gdd.md` — Unity Migration. Scope locked, Phase 1 (Agents 30-35) ready to build, in parallel with alpha; current active work alongside the alpha checklist above.
