...
**Read this file first in any session.** If you're about to implement a formula, add a data shape, or wire a scene together, check that system's contract in `docs/functional-agents/` first — it's kept current with the real code, organized by feature rather than by build phase. If no file exists there for the system you're touching (crew, notably, doesn't have one yet), fall back to the phase-based contract in `docs/agents/`. Either way, the contract defines exactly what you may and may not touch.
...
- **Migration to Unity: Phase 1 and Phase 2 (Sub-Phases A-F) both complete.** The original migration trigger ("once the core loop is fun and the systems are locked") has been evaluated and met — see `docs/profitable-unity-migration-gdd.md` for the full port-vs-rewrite split and sequencing, and `docs/unity-migration-phase2-checklist.md` for the followable Phase 2 task list, now independently re-audited (2026-08-04), not just trusted from the original completion claim. Migration proceeds **in parallel with Alpha**, not as a blocking prerequisite or a replacement: the existing web/Electron build stays live and deployable throughout, the same way Phase 2 didn't retire the MVP — confirmed via a full diff across every Unity-tagged commit, not assumed. **Unity MCP, the GDD's own stated tooling plan (see that GDD's §1), was never actually set up** — a real, disclosed environment gap, not a design choice. All Unity Editor work instead went through C# runtime scene construction (`MvpLoopBootstrap.cs` and each panel script) plus one `-executeMethod` scene-baking script (`Assets/Editor/SceneBuilder.cs`), not interactive Editor-driven authoring.
...
**Multiplayer backend timing decision revisited (post-Electron packaging) — deferred further.** Packaging exists but is Windows-only (macOS blocked on hardware/credentials), and the underlying tuning values are still explicitly provisional pending the feel-tuning pass (Section 2 of the alpha plan), so the "app, not web-driven" trigger condition isn't fully in spirit yet even though the mechanics exist. Re-evaluate after the feel-tuning playtest pass completes, not before.

**TradeWars 2000/2002 is this project's confirmed foundational gameplay-loop inspiration** — `docs/profitable-tradewars-alignment.md` is a researched comparison (not a locked design pass) against Profitable's current systems. It found one **verified, real bug**: single-player trading has no permanent counterparty once the one-time seed-market listings sell out or expire (`purchaseListing()`'s self-trade check permanently blocks a solo player from ever buying their own listing, and nothing else exists to buy it either) — see `docs/functional-agents/planetary-markets.md`'s own flagged gap. It also names several larger open questions not yet decided (planet ownership/colonists/citadels, a session-scarce resource beyond Fuel, reputation/alignment) and several places Profitable already deliberately diverged from TW2002 on purpose (component-built ships, shallow single-roll combat, async-only multiplayer) — read it before starting new work on Trading, Planet, or Ship.

**Seven systems were given a locked design; six are implemented, one (Citadels) was subsequently cut from alpha scope: Ship Fuel, Cargo Hold Capacity, Planet Resource Generation, Ship Crew Roles, Trading Counterparty, Colonist-Driven Production, and ~~Citadels~~.** All seven are documented in `profitable-design-questions.md` (sections of the same names, Citadels carrying a retroactive-removal note) and the six built ones are consolidated into forward-looking contracts in `docs/functional-agents/ship.md`, `planet.md`, `planet-ownership.md`, `planetary-markets.md`, and `gathering.md`. Unlike Multiplayer, these are **tracked as new Alpha checklist items** (see `docs/product-alpha.md`), not deferred. First four: Ship Fuel gates voyages on a new per-tier fuel resource; Cargo Hold Capacity caps the existing-but-never-wired-up `Voyage.cargo` field; Planet Resource Generation replaces gathering's per-action random quality roll with one fixed quality per planet-resource (rolled once, reset on a periodic cycle) plus a starting-planet tutorial guarantee — **extended (2026-08-05) with Per-Resource Quantity Caps**, a tier-scaled max-units-per-cycle ceiling on top of the existing binary colonist gate (the same 3 tutorial-guaranteed resources stay uncapped), so a colonized planet's resources are no longer infinitely gatherable within a cycle; see `docs/functional-agents/planet.md`'s own section for the full account; Ship Crew Roles lets already-hired crew fill new Pilot/Combat Engineer/Science Officer/Systems Engineer/Crafter slots on a ship, scaling by ship tier. Last three, from the TradeWars alignment review: Trading Counterparty adds `sellToMarket()`, an instant listing-free sale, closing the verified no-counterparty bug; Colonist-Driven Production gates a planet's gatherability behind a one-time colonist investment (the starting planet excepted); **Citadels — let a colonized planet be claimed and developed (3 levels — repair, refuel discount, deliberately no defense mechanic) — was fully built, tested, and Unity-ported, then retroactively removed (2026-08-04) as an alpha scope reduction, not a design or implementation flaw.** See `docs/functional-agents/planet-ownership.md`'s own retroactive-removal note for the full account. None of the seven touch `refine()`/`craft()`'s own formulas from Section 3.

**Active now: Alpha.** With all four deferred gaps resolved (three built, Multiplayer designed-but-deferred), the project pivoted to turning the verified proof-of-concept into a first-playable alpha rather than building Multiplayer's backend immediately. Full plan lives in `docs/product-alpha-plan.md` (rationale) and `docs/product-alpha.md` (the trackable checklist), with detail docs `docs/profitable-alpha-tuning-values.md`, `docs/profitable-alpha-scale-performance-plan.md`, `docs/profitable-alpha-uiux-onboarding-plan.md`, and `docs/profitable-alpha-electron-plan.md`. Five sequenced sections (each depends on the one before), plus a sixth, parallel-track section added later: **1. Content Authoring** (21 resources, 10 refining recipes, 13 crafting recipes, 16 ship component recipes, schematics, crew professions, ship-build presets — current content set is only the original MVP/milestone placeholders, nowhere near enough to actually play) → **2. Balance Tuning** (every numeric value is a functionally-correct placeholder, never tuned through real play) → **3. Scale & Performance** (fix galaxy at 50 planets, verify map/travel rendering and `getGlobalPrice()` performance at that scale) → **4. UI/UX & Onboarding** (skippable tutorial flow, settings screen, debug/tuning panel, tier-color legibility pass) → **5. Electron Packaging** (wrap the existing Vite/Phaser build, swap `SaveSystem` to filesystem access, package for Windows/macOS). Electron packaging's last item is a deliberate decision point: whether to begin Multiplayer backend work once the app-conversion milestone is reached, or keep deferring it. **6. Design Amendments** (Ship Fuel, Cargo Hold Capacity, Planet Resource Generation, Ship Crew Roles — see Section 6 above of this file) doesn't block or get blocked by 1-5; not required for Alpha-Ready per `product-alpha.md`'s own explicit call on that.

**Migration Phase 1 is complete, running in parallel with Alpha.** Per `docs/profitable-unity-migration-gdd.md` — see Section 2 above for how this relates to Alpha (parallel tracks, neither blocks the other). Agents 31-36 (renumbered from the GDD's original 30-35 draft — Agent 30 was already claimed by `agent-30-alpha-content-confirmation.md`; not a problem, the doc now matches reality rather than the original draft numbering) ported the MVP core loop (`rollQuality`, `getTierColor`, `refine`, `craft`, `loadContent`) to C#, proved numeric parity against the existing TypeScript test suite (343 cross-executed cases plus an end-to-end integration proof), and built a real Unity scene (`unity/ProfitableUnity/`, Unity 6000.5.6f1) a player can open and click through the full gather→refine→craft loop. All six contracts are written: `docs/agents/agent-31-unity-data-schema.md` through `agent-36-unity-migration-phase1-integration.md`.

**Migration Phase 2 (Sub-Phases A-F: Galaxy/Planet/Mining, Trading, Crew, Ships & Travel, Planet Ownership, Encounters & Combat) is also complete** — full followable task list and per-sub-phase agent rosters in `docs/unity-migration-phase2-checklist.md`. Unlike Phase 1's original completion claim, Phase 2's was independently re-audited afterward (2026-08-04) rather than just trusted, and that audit found and fixed real gaps rather than only confirming the original claim:

- **A third fractional-boundary bug**, same class as two bugs already documented in `profitable-mvp-gdd.md` (`getTierColor()`, `penaltyCurve.ts` — both compare a continuous/fractional value against integer-only band logic). The third — `HazardFailureCostCurve`'s band lookup in `resolveEncounters.ts`/`ResolveEncounters.cs` — was found specifically during this Unity migration parity audit, not previously known on either side of the codebase. All three are now fixed in lockstep across TS and C#, with regression tests in both languages: the first two documented in `profitable-mvp-gdd.md`'s own retroactive correction notes, the third in `profitable-travel-encounters-gdd.md`'s and `docs/unity-migration-phase2-checklist.md`'s own.
- **A starting-credits porting error**: Unity's `MarketState.StartingCredits` had drifted to 100 instead of the real TS source's 500 (`tradingState.ts`'s `STARTING_CREDITS`) — a genuine "port, don't redesign" violation, now corrected. 500 is formally logged in `docs/profitable-alpha-tuning-values.md` as a previously-undocumented value, accepted as-is since it was implicitly validated by the already-completed feel-tuning playtest pass.
- **Three presentation-completeness gaps closed**: Refine/Craft recipe selectors (the full real roster, not one hardcoded recipe each), map-based travel to any real galaxy planet, and Market buy capability (with proven self-trade-prevention).
- **Scanner UI built and the discovery-gate follow-up closed together (2026-08-04, later same day)**: scanner purchase (`ShipsPanel.cs`) and scan (`MapPanel.cs`) now wire the already-ported `ScannerPurchaser`/`ScanPerformer`. Map travel's own `Discovered`-flag gate (temporarily left open when the travel gap above was first closed, since nothing could discover a planet yet) is re-applied, matching the real TS build's `TradeMapScene.ts` exactly — the two were closed as one piece of work, not separately.
- **Confirmed still absent, all pre-existing documented scope cuts, not new gaps**: ~~ship component crafting, ship assembly/component-swap UI~~ (**resolved 2026-08-08** — Ship Assembly UI, see below), ~~the debug/tuning panel~~ (**resolved 2026-08-08**, see below). **"Won-combat return-voyage UI" removed from this list entirely (2026-08-08) — investigated and confirmed a false gap, not a real scope cut.** `agent-63-unity-encounters-combat-presentation.md`'s and `agent-64-unity-encounters-combat-integration.md`'s own Must-Not-Do sections ("must not build a return-voyage UI for a normal combat win") were a scope *guardrail* aimed at the agents building this sub-phase, not a report of a missing feature — per the locked Combat design (`profitable-design-questions.md`'s own Combat section: "fight-and-win: continue to original destination, no damage... winning grants nothing beyond reaching the original destination"), a win never redirects the voyage at all, confirmed directly in `ResolveCombatChoice.cs`'s `won` branch, which returns `RetreatVoyage = null` and the ship unchanged. Only lose/flee ever produce a retreat voyage. **With this correction, the Unity presentation gap list is fully closed** — every real item (Refine/Craft recipe selectors, map-based travel, Market buy, Scanner UI, Ship Assembly UI, Ship Crew Roles UI, Debug/Tuning Panel) is resolved.
- **Real regression introduced by the later Citadels removal (2026-08-04, still later the same day): Unity's `> Check Repair` is now a no-op for every ship.** Ship Crew Roles assignment UI was never built in Unity (a pre-existing, already-documented scope cut, listed above); Citadel repair was the one rate source that let `> Check Repair` still do something despite that gap. With Citadels cut (see this section's own paragraph above, and `docs/functional-agents/planet-ownership.md`'s retroactive-removal note), `ComponentRepairResolver.ResolveComponentRepair()` has zero eligible rate sources left in Unity — it stamps `Ship.LastRepairedAt` but restores no durability. TS is unaffected (`ShipStatusScene` has real crew-role assignment UI there). Full account: `docs/unity-migration-phase2-checklist.md`'s own note on this, and the comment at `unity/ProfitableUnity/Assets/Scripts/UI/ShipsPanel.cs:280-289`. Not fixed as part of the removal — the fix is building Ship Crew Roles assignment UI in Unity, separately-scoped deferred work, not a quick patch. **Resolved (2026-08-08)**: new `ShipCrewRolesPanel.cs` ports `ShipStatusScene.ts`'s real assign/unassign flow for all 5 roles; `CheckRepair()`/`ResolveCombatChoice` needed no code changes since they already passed the real crew list — verified via real click-through that an assigned Systems Engineer now makes `> Check Repair` restore real durability (50 → 100 in one measured run). Pilot/Science Officer get the same real assignment UI but are deliberately left unwired into travel/scan, faithfully matching a real gap in the TypeScript source itself (`TradeMapScene.ts`'s own call sites never pass a resolved pilot/scienceOfficer either). Full account: `docs/unity-migration-phase2-checklist.md`'s own resolution note.
- **Housekeeping**: a stray 2.3GB nested git clone at `unity/ProfitableUnity/profitable/` was found and removed, with a `.gitignore` rule added to prevent recurrence.
- **Debug/tuning panel gap closed (2026-08-08), both engines.** TS's own `debugTuningRegistry.ts` was found stale first (missing Ship Fuel, Cargo Hold Capacity, all 5 Ship Crew Role effect tables, Colonist-Driven Production, and Per-Resource Quantity Caps -- 16 real tunables with no panel row at all) and fixed there before Unity was built to match it, per this project's "faithful port of TS's real behavior" rule -- staleness is a genuine content gap, not a deliberate design limitation, so it was corrected rather than faithfully reproduced. `PlanetOwnershipConstants.MinimumColonistsToProduce`/`PlanetResourceCycleConstants.PlanetResourceResetIntervalHours` were converted from `const` to the same mutable-property convention every other tunable already uses (zero other files needed to change -- both were plain runtime reads, no compile-time-constant usage). A new `DebugPanel.cs` covers the complete real tunable set (Trading/Crew/Ships & Travel/Ship Crew Roles/Planet & Colonists/Scanner/Travel Encounters/Combat -- every scalar and per-tier table, `-`/`+` steppers), reachable only via a new `DebugGate` (Editor: `Application.isEditor`; standalone: a `FileSaveSystem`-persisted flag toggled by a Ctrl+Shift+D shortcut, requiring an actual process restart to re-check, matching TS's own `location.reload()` pattern). Force Encounter (`ForcedEncounterRandom.cs`/`DebugState.cs`, porting `debugForcedRandom.ts`/`debugState.ts`) is wired into `ShipsPanel.ResolveArrival()`'s existing `RandomFn` seam. Verified real-click-through on both engines (TS via a live dev-server browser session reading the real constants modules before/after each click; Unity via `button.onClick.Invoke()` on the real, panel-constructed `Button`, same standard `ShipCrewRolesPanelClickThroughTest.cs` already established) and gating verified against a real built Standalone Windows player (`-selfTestDebugGate` self-check: `DebugGateIsEnabled=False`/`DebugPanelConstructed=False` by default, both `True` after the persisted flag is set and the process restarts). Full counts: `npm test` 681/681 (unchanged -- no TS test file covers the tuning registry), `dotnet test` 742/742 (unchanged), Unity EditMode 104->139/139 (+35), Unity PlayMode 9/9 (unchanged).

Current verified counts as of the original Phase 2 audit (2026-08-04): `dotnet test` 748/748, `npm test` 688/688, Unity EditMode 82/82, Unity PlayMode 6/6 (stable across repeated runs). **Since superseded by the Citadels removal, same day** (see this section's own paragraph above): `dotnet test` 729/729, `npm test` 666/666, confirmed; Unity EditMode/PlayMode were not independently re-run in this environment (no Unity Editor available here). The existing web/Electron build is confirmed completely unmodified by all Unity migration work, verified via a full diff across every Unity-tagged commit.
...
- Any new gameplay system beyond what's already designed — alpha is about making the *existing* design playable, not extending it. (Unity migration is no longer in this bullet: its GDD is scope-locked and Phase 1 is ready to build, running in parallel with alpha per Section 2 above — not blocked by alpha scope, and not itself alpha scope either. See `docs/profitable-unity-migration-gdd.md`.)
...
- `docs/functional-agents/README.md` — feature-organized companion to `docs/agents/`, documenting each system's *current* contract (Ship, Galaxy, Planet, Gathering, Refining, Recipes/Schematics, Crafting, Planetary Markets, Galactic Market, Travel, Encounters/Combat, Universe). Additive, not a replacement — check here first per Section 1 above.
- `docs/profitable-tradewars-alignment.md` — researched comparison against TradeWars 2000/2002, this project's confirmed foundational inspiration. Found one verified real bug (no permanent trading counterparty in solo play) and flags several open questions (planet ownership, a turns-like scarce resource, reputation) — not a locked design pass, read before deciding whether to act on any of it.
...
- `docs/profitable-unity-migration-gdd.md` — Unity Migration. Scope locked; Phase 1 (Agents 31-36) complete, running in parallel with alpha.
- `docs/unity-migration-phase2-checklist.md` — Unity Migration Phase 2, the followable task list `profitable-unity-migration-gdd.md` §2 scopes as "future GDDs, not scoped here." Sub-Phases A-F all complete, independently re-audited 2026-08-04.
