...
- **Migration to Unity: scope locked, Phase 1 ready to build.** The original migration trigger ("once the core loop is fun and the systems are locked") has been evaluated and met — see `docs/profitable-unity-migration-gdd.md` for the full port-vs-rewrite split, sequencing, and the Phase 1 agent roster (Agents 31-36). Migration proceeds **in parallel with Alpha**, not as a blocking prerequisite or a replacement: the existing web/Electron build stays live and deployable throughout, the same way Phase 2 didn't retire the MVP. Migration Phase 1's scope (simulation core + minimal presentation for gather/refine/craft) doesn't overlap with Alpha's remaining sections, so either can proceed first.
...
- `docs/profitable-unity-migration-gdd.md` — Unity Migration. Scope locked, Phase 1 (Agents 31-36) ready to build, in parallel with alpha; current active work alongside the alpha checklist above.
