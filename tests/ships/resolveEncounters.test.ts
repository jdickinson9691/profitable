...
test("a voyage spanning N windows gets N independent trigger rolls, not one roll for the whole voyage", () => {
  ...
  const results = resolveEncounters(voyage(3), ship(), planet(), resources, random);
...
test("a voyage shorter than one full window still gets at least one roll", () => {
  ...
  const results = resolveEncounters(shortVoyage, ship(), planet(), resources, random);
...
test("type distribution over many trials roughly matches ENCOUNTER_TYPE_WEIGHTS, with hazard genuinely least common", () => {
  ...
  for (let i = 0; i < trials; i++) {
    ...
    const results = resolveEncounters(voyage(1), ship(), planet(), resources, random);
...
test("tradeOpportunity: exact credits amount from a known roll, and every trial across many stays within the documented range", () => {
  ...
  const [result] = resolveEncounters(voyage(1), ship(), planet(), resources, random);
  ...
  for (let i = 0; i < 200; i++) {
    ...
    const [trial] = resolveEncounters(voyage(1), ship(), planet(), resources, trialRandom);
...
test("discovery: an empty eligible pool produces no result for that window rather than throwing", () => {
  ...
  const results = resolveEncounters(voyage(1), ship(), emptyPoolPlanet, resources, random);
  assert.equal(results.length, 0);
});
...
test("hazard: the pass/fail roll is modified by the voyage's ship's derived tier -- an identical raw roll fails for Grey but passes for Gold", () => {
  // trigger (0), type roll landing on hazard (0.9, >= 0.8), raw roll of 40
  // (1-100 scale: floor(0.39 * 100) + 1 = 40) -- Grey's +0 bonus keeps it
  // below HAZARD_PASS_THRESHOLD (50); Gold's +30 bonus pushes it to 70, a pass.
  const rawRoll = 0.39;

  const greyResult = resolveEncounters(voyage(1), ship("Grey"), planet(), resources, queueRandom([0, 0.9, rawRoll])) as HazardEncounterResult[];
  assert.equal(greyResult[0]!.type, "hazard");
  assert.equal(greyResult[0]!.outcome.passed, false);

  const goldResult = resolveEncounters(voyage(1), ship("Gold"), planet(), resources, queueRandom([0, 0.9, rawRoll])) as HazardEncounterResult[];
...
test("hazard: failure cost curve matches the escalating shape exactly at several distance-below-threshold values", () => {
  ...
  for (const { rawRoll, expectedMultiplier } of cases) {
    ...
    const [result] = resolveEncounters(voyage(1), ship("Grey"), planet(), resources, random);
